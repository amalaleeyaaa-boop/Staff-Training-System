...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $check = mysqli_query($conn, "SELECT course_id FROM courses WHERE course_id = '$course_id' LIMIT 1");
    if (mysqli_num_rows($check) > 0) {
        $message = "<div class='w3-panel w3-red w3-round-large w3-padding-16 w3-margin-bottom-24'>❌ Error: Course ID $course_id is already in use! Please use a different ID.</div>";
    } else {
        $sql = "INSERT INTO courses (course_id, course_name, lecturer, description, start_date, end_date)
                VALUES ('$course_id', '$course_name', '$lecturer', '$description', '$start_date', '$end_date')";

        if (mysqli_query($conn, $sql)) {
            header("Location: list_courses.php?success=added");
            exit;
        } else {
...
<head>
    ...
    <style>
        ...
        .btn-save {
            width: 100%; padding: 16px; background: #28a745; border: none;
...
<div class="top-bar">
    <h1><i class="fa fa-book"></i> Add New Course</h1>
    <div>
...
<div class="form-container">
    ...
    <form method="post">
        ...
        <input type="number" name="course_id" placeholder="e.g. 101" required>

        <label>Course Name *</label>
        <input type="text" name="course_name" placeholder="e.g. Project Management" required>

        <label>Lecturer / Instructor *</label>
        <input type="text" name="lecturer" placeholder="e.g. Mr. Ahmad" required>

        <label>Course Description</label>
        <textarea name="description" placeholder="Enter a brief overview of this course"></textarea>
        ...
        <button type="submit" class="btn-save">
            <i class="fa fa-save"></i> Save Course Details
        </button>
    </form>
</div>
